...
1. Eat in right order: veg/fibre first, then protein/fat, then carbs/sugar last
...
KEY PAIRINGS: Rice/pasta eat veg+protein first | Fruit after meals | Oats add nut butter | Sushi edamame/miso first
`;

const UPF_KNOWLEDGE = `
# ULTRA PROCESSED FOOD (van Tulleken)
...
- No markdown. No bold. No asterisks. No headers. No code blocks. No backticks. No JSON.
- Do not add a title line at the start.
- Each score must be a single integer only — not "9/10", just "9"
- Start your response with the first field on the very first line, nothing before it.

RECIPE REQUESTS (when Dan mentions ingredients, asks for recipe ideas, or asks what to make):
RECIPE: true
INTRO: [1 witty sentence]
OPTION_1_NAME: [recipe name]
OPTION_1_ZOE: [1-10]
OPTION_1_GG: [1-10]
OPTION_1_UPF: [1-10]
OPTION_1_PROTEIN_G: [grams per serving]
OPTION_1_VERDICT: [1 sentence]
OPTION_1_METHOD: [3-5 steps, each on new line starting with a number]
OPTION_1_TIP: [one glucose/nutrition tip]
OPTION_2_NAME: [recipe name]
OPTION_2_ZOE: [1-10]
OPTION_2_GG: [1-10]
OPTION_2_UPF: [1-10]
OPTION_2_PROTEIN_G: [grams per serving]
OPTION_2_VERDICT: [1 sentence]
OPTION_2_METHOD: [3-5 steps]
OPTION_2_TIP: [one tip]
BEST_PICK: [name of better option and one sentence why]
Always suggest constructive options. Never just say everything is bad.
...
Scoring: ZOE=gut/fat (10=great), GG=glucose spike for Dan (strict, 10=no spike), UPF=processing (10=whole food). Tips = food/eating order only.`;
...
function parseRecipe(text) {
  ...
  const bestPick = text.match(/BEST_PICK:\s*([\s\S]+?)(?=OPTION_\d|$)/)?.[1]?.trim();
  const options = [];
  for (let n = 1; n <= 3; n++) {
    const name = text.match(new RegExp("OPTION_" + n + "_NAME:\\s*(.+)"))?.[1]?.trim();
    if (!name) break;
    const zoe = parseInt(text.match(new RegExp("OPTION_" + n + "_ZOE:\\s*(\\d+)"))?.[1] || "5");
    const gg = parseInt(text.match(new RegExp("OPTION_" + n + "_GG:\\s*(\\d+)"))?.[1] || "5");
    const upf = parseInt(text.match(new RegExp("OPTION_" + n + "_UPF:\\s*(\\d+)"))?.[1] || "5");
    const proteinG = parseInt(text.match(new RegExp("OPTION_" + n + "_PROTEIN_G:\\s*(\\d+)"))?.[1] || "0");
    ...
    const avg = Math.round((zoe + gg + upf) / 3);
    options.push({ name, zoe, gg, upf, proteinG, verdict, steps, tip, avg });
  }
...
function parseReply(text) {
  ...
  const z = clean.match(/ZOE_SCORE:\s*(\d+)/)?.[1];
  const g = clean.match(/GG_SCORE:\s*(\d+)/)?.[1];
  const u = clean.match(/UPF_SCORE:\s*(\d+)/)?.[1];
...
function CookMode({ recipe, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#1b4332", display: "flex", flexDirection: "column", zIndex: 1000, padding: "0 0 24px" }}>
      <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, fontWeight: 600 }}>COOK MODE</div>
          <div style={{ color: "white", fontWeight: 900, fontSize: 16 }}>{recipe.name}</div>
        </div>
        <button onClick={onClose} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "white", borderRadius: 12, padding: "8px 14px", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>Exit</button>
      </div>
      <div style={{ padding: "0 20px 10px", flexShrink: 0 }}>
        <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 10, height: 6 }}>
          <div style={{ background: "#00c875", height: 6, borderRadius: 10, width: pct + "%", transition: "width 0.4s" }} />
        </div>
        <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 6 }}>Step {step + 1} of {total}</div>
      </div>
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#00c875", color: "white", fontSize: 24, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>{step + 1}</div>
          <div style={{ color: "white", fontSize: 24, fontWeight: 700, lineHeight: 1.5 }}>{recipe.steps[step]}</div>
        </div>
      </div>
      {recipe.tip && isLast && (
        <div style={{ margin: "0 20px 16px", background: "rgba(255,224,0,0.15)", border: "1px solid rgba(255,224,0,0.3)", borderRadius: 16, padding: "12px 16px" }}>
          <div style={{ color: "#ffe066", fontSize: 13, fontWeight: 600 }}>💡 {recipe.tip}</div>
        </div>
      )}
      <div style={{ display: "flex", gap: 12, padding: "0 20px", flexShrink: 0 }}>
        {step > 0 && (
          <button onClick={() => setStep(s => s - 1)} style={{ flex: 1, padding: "16px", background: "rgba(255,255,255,0.15)", color: "white", border: "none", borderRadius: 16, cursor: "pointer", fontSize: 16, fontWeight: 700 }}>Back</button>
        )}
        {!isLast ? (
          <button onClick={() => setStep(s => s + 1)} style={{ flex: 2, padding: "16px", background: "#00c875", color: "white", border: "none", borderRadius: 16, cursor: "pointer", fontSize: 16, fontWeight: 700 }}>Next step</button>
        ) : (
          <button onClick={onClose} style={{ flex: 2, padding: "16px", background: "#00c875", color: "white", border: "none", borderRadius: 16, cursor: "pointer", fontSize: 16, fontWeight: 700 }}>Done!</button>
        )}
      </div>
    </div>
  );
...
function RecipeCard({ data }) {
  ...
  return (
    <div style={{ maxWidth: "96%", alignSelf: "flex-start", display: "flex", flexDirection: "column", gap: 8 }}>
      {cookMode && <CookMode recipe={cookMode} onClose={() => setCookMode(null)} />}
      {data.intro && (
      ...
      {data.options.map((opt, i) => {
        const avgColor = opt.avg >= 7 ? "#00c875" : opt.avg >= 4 ? "#ffb800" : "#ff4444";
        const isOpen = open === i;
        const pct = Math.round((opt.proteinG / 140) * 100);
        const proteinColor = opt.proteinG >= 20 ? "#00c875" : opt.proteinG >= 10 ? "#ffb800" : "#ff4444";
        return (
          <div key={i} style={{ background: "white", borderRadius: 20, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", border: isOpen ? "2px solid " + avgColor : "2px solid transparent" }}>
            <button onClick={() => setOpen(isOpen ? -1 : i)} style={{ width: "100%", background: "none", border: "none", padding: "14px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, textAlign: "left" }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: avgColor + "22", border: "2px solid " + avgColor, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 18, color: avgColor, flexShrink: 0 }}>{opt.avg}</div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 14, color: "#222" }}>{opt.name}</div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{opt.verdict}</div>
                </div>
              </div>
              <div style={{ fontSize: 16, color: "#ccc", flexShrink: 0 }}>{isOpen ? "▲" : "▼"}</div>
            </button>
            {isOpen && (
              <div style={{ padding: "0 16px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-around", padding: "12px 0", borderTop: "1px solid #f0f0f0", borderBottom: "1px solid #f0f0f0", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                  <ScoreCircle label="Zoe" score={opt.zoe} icon="🧬" />
                  <ScoreCircle label="Glucose" score={opt.gg} icon="🩸" />
                  <ScoreCircle label="UPF" score={opt.upf} icon="🏭" />
                  {opt.proteinG > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                      <div style={{ background: proteinColor + "22", border: "2px solid " + proteinColor, borderRadius: 14, padding: "6px 10px", textAlign: "center", minWidth: 52 }}>
                        <div style={{ fontSize: 18, fontWeight: 900, color: proteinColor, lineHeight: 1 }}>{opt.proteinG}g</div>
                        <div style={{ fontSize: 9, color: proteinColor, fontWeight: 700 }}>{pct}%</div>
                      </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                  {opt.steps.map((s, si) => (
                    <div key={si} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                      <div style={{ width: 26, height: 26, borderRadius: "50%", background: avgColor, color: "white", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{si + 1}</div>
                      <div style={{ fontSize: 13, color: "#444", lineHeight: 1.6, paddingTop: 3 }}>{s}</div>
                    </div>
                  ))}
                </div>
                {opt.tip && (
                  <div style={{ background: "#fffbea", border: "1px solid #ffe066", borderRadius: 12, padding: "8px 12px", fontSize: 12, color: "#886600", marginBottom: 12 }}>💡 {opt.tip}</div>
                )}
                <button onClick={() => setCookMode(opt)} style={{ width: "100%", padding: "13px", background: "linear-gradient(135deg,#1b4332,#40916c)", color: "white", border: "none", borderRadius: 14, cursor: "pointer", fontSize: 15, fontWeight: 800 }}>
                  👨‍🍳 Cook this
                </button>
              </div>
            )}
...
function ResultCard({ m }) {
  ...
  const vStyle = avg >= 7
    ? { label: "Great choice! 🎉", bg: "linear-gradient(135deg,#00b865,#00c875)" }
    : avg >= 4
    ? { label: "OK in moderation ⚠️", bg: "linear-gradient(135deg,#f0a500,#ffb800)" }
    : { label: "Best avoided ❌", bg: "linear-gradient(135deg,#e03030,#ff4444)" };
    ...
    <div style={{ maxWidth: "92%", alignSelf: "flex-start", display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ background: "white", borderRadius: 20, padding: "16px 12px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", display: "flex", justifyContent: "space-around", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        ...
        {p.proteinG > 0 && (() => {
          ...
          const lbl = p.proteinG >= 20 ? "High 💪" : p.proteinG >= 10 ? "Moderate" : "Low";
          return (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
...
function LockScreen({ onUnlock }) {
  ...
  function handleTap(e) {
    ...
    setTaps(newTaps);
    if (newTaps.length >= 3) { onUnlock(); return; }
    timerRef.current = setTimeout(() => setTaps([]), 2000);
  }
  return (
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "system-ui,sans-serif", maxWidth: 600, margin: "0 auto", height: "100dvh", display: "flex", flexDirection: "column", background: "#f5f8f5" }}>
            ...
            <div key={i} style={{ maxWidth: "80%", alignSelf: "flex-end", background: "linear-gradient(135deg,#1b4332,#2d6a4f)", color: "white", borderRadius: "20px 20px 4px 20px", padding: "12px 16px", boxShadow: "0 2px 10px rgba(27,67,50,0.3)" }}>
              {m.previews?.length > 0 && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: m.text ? 8 : 0 }}>
                  {m.previews.map((p, pi) => (
                    <img key={pi} src={p} style={{ height: 80, width: "auto", borderRadius: 10, border: "2px solid rgba(255,255,255,0.3)" }} />
                  ))}
                </div>
              )}
      ...
      <div style={{ padding: "10px 14px 16px", background: "white", borderTop: "2px solid #eef2ee", flexShrink: 0, position: "sticky", bottom: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          ...
          <textarea
            ...
            placeholder="e.g. oat milk, avocado toast, what can I make with eggs..."
          ...
          <button
            ...
            style={{ flexShrink: 0, width: 48, height: 48, background: (!input.trim() && images.length === 0) ? "#ddd" : "linear-gradient(135deg,#1b4332,#40916c)", color: "white", border: "none", borderRadius: 16, cursor: (!input.trim() && images.length === 0) ? "not-allowed" : "pointer", fontSize: 20, boxShadow: (!input.trim() && images.length === 0) ? "none" : "0 3px 10px rgba(27,67,50,0.3)", transition: "all 0.2s" }}
          >↑</button>
        </div>
        <div style={{ textAlign: "right", fontSize: 9, color: "#ccc", marginTop: 4 }}>v21</div>
      </div>
